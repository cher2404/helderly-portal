  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-2xl font-semibold text-slate-100">Abonnementen</h1>
        <a
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-medium text-slate-300">Actieve abonnementen ({withSubscription.length})</h2>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-slate-500/20">
                ...
                <th className="px-4 py-3 font-medium text-slate-400">Stripe Customer ID</th>
              </tr>
            </thead>
            <tbody>
              {withSubscription.map((p) => (
                <tr key={p.id} className="border-b border-slate-500/10 last:border-0">
                  <td className="px-4 py-3 text-slate-100">{p.full_name || "—"}</td>
      ...
      <section className="space-y-3">
        <h2 className="text-lg font-medium text-slate-300">In trial ({trial.length})</h2>
        <p className="text-sm text-slate-500">
      ...
      <p className="text-sm text-slate-500">
        Webhook <code className="rounded bg-slate-800 px-1">checkout.session.completed</code> en{" "}
        <code className="rounded bg-slate-800 px-1">customer.subscription.updated/deleted</code> zorgen voor sync tussen Stripe en de profiles tabel.
      </p>
    </div>
  );
}
